...
function assertIsBignum(arg) {
  ...
  assert_(arg.shiftRight, "arg might not be a bignum");
}
 ...
 * returns: bignum
 */
function server_getS(params, salt, v, A, b) {
  assertIsBignum(salt);
  assertIsBignum(v);
  assertIsBignum(A);
  assertIsBignum(b);
...
module.exports = {
  ...
  server_getS: function (params, salt, v, A, b) {
    return server_getS(params, bignum.fromBuffer(salt), bignum.fromBuffer(v),
                       bignum.fromBuffer(A), bignum.fromBuffer(b)).toBuffer();
  },
  getu: getuBuffer,
  getk: getkBuffer,
