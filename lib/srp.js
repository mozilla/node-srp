 ...
 * returns: buffer
 */
function makeVerifier(params, salt, I, P) {
  assertIsBuffer(salt, "salt (salt)");
  assertIsBuffer(I, "identity (I)");
  assertIsBuffer(P, "password (P)");
  var Vnum = params.g.powm(getx(params, salt, I, P), params.N);
...
function Client(params, salt_buf, identity_buf, password_buf, a_buf) {
...
function Server(params, verifier_buf, b_buf) {
  assertIsBuffer(verifier_buf, "verifier");
  assertIsBuffer(b_buf, "b");
  this._private = { params: params,
                    k_num: getk(params),
                    b_num: bignum.fromBuffer(b_buf),
