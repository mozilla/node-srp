...
function assertIsBuffer(arg, argname) {
  argname = argname || "arg";
  assert(Buffer.isBuffer(arg), "Type error: "+argname+" must be a buffer");
}

function assertIsNBuffer(arg, Nbuf, argname) {
  argname = argname || "arg";
  assert(Buffer.isBuffer(arg), "Type error: "+argname+" must be a buffer");
  assert.equal(arg.length, Nbuf.length);
}

function assertIsBignum(arg) {
  // I'm not actually sure how to test this.
  assert(arg.shiftRight, "arg might not be a bignum");
}
