 ...
 *
 * Note that P_stretch is the HKDF-stretched key, computed elsewhere.
 */
...
const inputs = {
    ...
    salt: hex('00f100000000000000000000000000000000000000000000000000000000009b'),
    b: decimal('  1198277 66042000 95785634 94115500 91527197 08125226 96054476'
               ...
               +'62611677 70805044 44704039 04739431 33561758 53336713 78812960'),
    a: decimal('  1193346 47663227 29136311 34057412 43413916 43482736 31461660'
               ...
               +'31489818 95205023 68799907 19946264 95152039 36244793 15530076')
};
...
const expected = {
    I_hex: '616e6472c3a9406578616d706c652e6f7267',
    ...
    x: decimal('115713340795669212831971819661984296758573939625477265918747447380376082294071'),
    v: decimal('   710597 15947322 36316881 86192315 96014948 50266475 38798215'
               ...
               +'46225452 67882212 38212661 40913290 18051354 03998520 50747986'),
    v_hex: join(' 00901a4e 05a7986c fafe2c80 993f6e21 847d38b8 b9168065 14948072'
                ...
                +'240c9252'),
    b_hex: join(' 00f30000 00000000 00000000 00000000 00000000 00000000 00000000'
                ...
                +'00000000 00000000 00000000 00000000 00000000 00000000 00000000'
                +'00000020'),
    B_hex: join(' 00857f70 b197a6f3 f79c4270 a41c581d 62c7ec7f c554c797 481d4b40'
                ...
                +'a51a5267'),
    a_hex: join(' 00f20000 00000000 00000000 00000000 00000000 00000000 00000000'
                +'00000000 00000000 00000000 00000000 00000000 00000000 00000000'
                +'00000000 00000000 00000000 00000000 00000000 00000000 00000000'
                +'00000000 00000000 00000000 00000000 00000000 00000000 00000000'
                +'00000000 00000000 00000000 00000000 00000000 00000000 00000000'
                ...
                +'00000000 00000000 00000000 00000000 00000000 00000000 00000000'
                +'0000115c'),
    A_hex: join(' 00f2a357 d7da7132 1be6c070 fb3a5928 8cec951c b13e7645 1f8c466a'
    ...
    S_hex: join(' 009cc8da 2f7a9501 5bc0091f aa36d6ef ff52c33b 924353e1 1de1d8e7'
...
.addBatch({
  'test vectors': {
    'I encoding': function() {
      assert.equal(inputs.I.toString('hex'), expected.I_hex);
    },

    'getk': function() {
      assert.equal(srp.getk(params.N, params.g, ALG).toString('hex'),
                   expected.k.toString('hex'));
    },

    'getx': function() {
      assert.equal(srp.getx(inputs.salt, inputs.I, inputs.P_stretch, ALG).toString('hex'), expected.x_hex);
    },

    'getv': function() {
      assert.equal(pad(srp.getv(inputs.salt, inputs.I, inputs.P_stretch, params.N, params.g, ALG).toString('hex')), expected.v_hex);
    },

    'b encoding': function() {
      assert.equal(pad(inputs.b.toString('hex')), expected.b_hex);
    },

    'a encoding': function() {
      assert.equal(pad(inputs.a.toString('hex')), expected.a_hex);
    },

    'getB (on server)': function() {
      var B = srp.getB(expected.v, params.g, inputs.b, params.N, ALG);
      assert.equal(pad(B.toString('hex')), expected.B_hex);
    },

    'getA (on client)': function() {
      var A = srp.getA(params.g, inputs.a, params.N);
      assert.equal(pad(A.toString('hex')), expected.A_hex);
    },

    'getu': function() {
      var A = Buffer(expected.A_hex, 'hex');
      var B = Buffer(expected.B_hex, 'hex');
      var u = srp.getu(A, B, params.N, ALG);
      assert.equal(u.toString('hex'), expected.u_hex);
    },
    ...
    'secrets': {
      'client': {
        topic: function() {
          var B = Buffer(expected.B_hex, 'hex');
          return srp.client_getS(inputs.salt, inputs.I, inputs.P_stretch, params.N, params.g, inputs.a, B, ALG);
        },

        'S': function(S) {
          assert.equal(pad(S.toString('hex')), expected.S_hex);
        },

        'K': function(S) {
          assert.equal(srp.getK(S, params.N, ALG).toString('hex'), expected.K_hex);
        }
      ...
      'server': {
        topic: function() {
          var A = Buffer(expected.A_hex, 'hex');
          return srp.server_getS(inputs.salt, expected.v, params.N, params.g, A, inputs.b, ALG);
        },
