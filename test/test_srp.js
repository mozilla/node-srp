...
.addBatch({
  "create Verifier": function() {
    verifier = srp.makeVerifier(params, salt, identity, password);
  ...
  "create a and b": {
    ...
    "use a and b": function() {
      ...
      server.checkM1(client.getM1());

      // client and server agree on K
      var client_K = client.getK();
      var server_K = server.getK();
    ...
    "server rejects wrong M1": function() {
      ...
      bad_client.setB(server2.getB());
      assert.throws(function(){server.checkM1(bad_client.getM1());},
